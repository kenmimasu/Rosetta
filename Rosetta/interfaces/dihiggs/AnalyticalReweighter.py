...
class AnalyticalReweighter(object):
    ...
    def TS_test(self, kl, kt, c2, cg, c2g):
        ...
        for bench in range(0,13) :
            EvByBinBM = np.absolute(
                          normEv*self.parameter_point(
                              self.klJHEP[bench],
                              self.ktJHEP[bench],
                              self.c2JHEP[bench],
                              self.cgJHEP[bench],
...
def reweighter_from_histogram_and_file():
        ...
        n_ev_V0_sum=np.loadtxt(data_dir+'/n_ev_V0_sum_59_4.out')
        ...
        data_from_file = np.genfromtxt(data_dir+"/coefficientsByBin_extended_3M_costHHSim_59-4.txt", names = column_names)
